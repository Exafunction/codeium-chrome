 ...
 */
export function numCodeUnitsToNumUtf8Bytes(text: string, numCodeUnits?: number): number {
  if (numCodeUnits === 0) {
    return 0;
  }
...
export function numUtf8BytesToNumCodeUnits(text: string, numUtf8Bytes?: number): number {
  if (numUtf8Bytes === 0) {
    return 0;
  }
  let curNumCodeUnits = 0;
